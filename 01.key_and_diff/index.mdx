...
##### 2. 生成 key 及确定挂载顺序顺序
  ...
  vnode 真正起作用的 key 是由其所在的层级关系及 key 属性共同决定，当 children 的某个元素 vnodeArray 是数组的时候，将生成一个层级前缀，取值为：
  
    (向上继承的层级前缀 || '.') + (vnode 的 key || vnode 在 children 中的索引)\

  因而 vnodeArray 内 vnode 真正的 key 的取值是：

    (层级前缀 ? 层级前缀 + ':' || '') + (vnode 的 key || vnode 在 vnodeArray 中的索引)\

  而 children 内其他 vnode 真正的 key 的取值则是：
